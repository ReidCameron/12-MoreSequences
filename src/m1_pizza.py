...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(220, 100),  # All numbers are approximate.
                rg.Point(200, 120),
                rg.Point(180, 100),
                rg.Point(200, 80),]
...
def run_test_pizza():
    ...
    # Tests 2 and 3 (on the same window):
    title = 'PIZZA tests 4:  16 black slices on yellow circle.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.fill_color = 'yellow'
    pizza(window, circle, 16, 'black', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        linee = rg.Line(circle.center,points[k])
        linee.color = color
        linee.thickness = thickness
        linee.attach_to(window)
        window.render(0.05)
...
def run_test_polygon():
    ...
    # Tests 3:
    title = 'POLYGON test 3:  5 segments with thick orange lines.'
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    circle.outline_thickness = 15
    polygon(window, circle, 5, 'orange', 12)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  100 lime green lines on deep pink circle, hops = 61.'
    window = rg.RoseWindow(500, 500, title)

    circle = rg.Circle(rg.Point(250, 250), 150)
    circle.fill_color = 'deep pink'
    fancy_polygon(window, circle, 300, 61, 'lime green', 8)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        pt = k+hops_to_next_point
        if pt > len(points)-1:
            pt = hops_to_next_point - (len(points) - k)
            line = rg.Line(points[k], points[pt])
        else:
            line = rg.Line(points[k], points[pt])
        line.thickness = thickness
        line.color = color
